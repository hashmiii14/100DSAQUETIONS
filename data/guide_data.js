...
const GUIDE_DATA = [
  ...
  },
  {
    ...
    "title": "27. FAANG Big-O Complexity & Decision Matrix",
    "category": "Interview Playbook",
    "summary": "Comprehensive Big-O time and space complexity cheat sheet across all fundamental data structures, operations, and algorithmic bounds.",
    "theory": "\n              <h3>Big-O Complexity Summary Table</h3>\n              <table style=\"width: 100%; border-collapse: collapse; margin: 12px 0; font-size: 13px;\">\n                <thead>\n                  <tr style=\"background: var(--bg-subtle); border-bottom: 1px solid var(--border-color); text-align: left;\">\n                    <th style=\"padding: 8px;\">Data Structure</th>\n                    <th style=\"padding: 8px;\">Access</th>\n                    <th style=\"padding: 8px;\">Search</th>\n                    <th style=\"padding: 8px;\">Insertion</th>\n                    <th style=\"padding: 8px;\">Deletion</th>\n                    <th style=\"padding: 8px;\">Space</th>\n                  </tr>\n                </thead>\n                <tbody>\n                  <tr style=\"border-bottom: 1px solid var(--border-color);\"><td><strong>Array / Vector</strong></td><td>O(1)</td><td>O(N)</td><td>O(N) (O(1) amortized end)</td><td>O(N)</td><td>O(N)</td></tr>\n                  <tr style=\"border-bottom: 1px solid var(--border-color);\"><td><strong>Singly / Doubly Linked List</strong></td><td>O(N)</td><td>O(N)</td><td>O(1) (known node)</td><td>O(1) (known node)</td><td>O(N)</td></tr>\n                  <tr style=\"border-bottom: 1px solid var(--border-color);\"><td><strong>Hash Table (HashMap / HashSet)</strong></td><td>N/A</td><td>O(1) avg / O(N) worst</td><td>O(1) avg / O(N) worst</td><td>O(1) avg</td><td>O(N)</td></tr>\n                  <tr style=\"border-bottom: 1px solid var(--border-color);\"><td><strong>Binary Search Tree (Balanced AVL/Red-Black)</strong></td><td>O(log N)</td><td>O(log N)</td><td>O(log N)</td><td>O(log N)</td><td>O(N)</td></tr>\n                  <tr style=\"border-bottom: 1px solid var(--border-color);\"><td><strong>Binary Heap (PriorityQueue)</strong></td><td>O(1) (peek min/max)</td><td>O(N)</td><td>O(log N)</td><td>O(log N) (poll)</td><td>O(N)</td></tr>\n                  <tr style=\"border-bottom: 1px solid var(--border-color);\"><td><strong>Trie (Prefix Tree)</strong></td><td>O(L) (word length)</td><td>O(L)</td><td>O(L)</td><td>O(L)</td><td>O(N * L * Alphabet)</td></tr>\n                  <tr style=\"border-bottom: 1px solid var(--border-color);\"><td><strong>Disjoint Set Union (DSU)</strong></td><td>N/A</td><td>O(α(N)) ≈ O(1)</td><td>O(α(N)) (Union)</td><td>N/A</td><td>O(N)</td></tr>\n                </tbody>\n              </table>\n              <h3>Common Interview Time Bounds & Constraints</h3>\n              <ul>\n                <li><strong>N ≤ 10–12:</strong> Exponential $O(2^N)$ or Factorial $O(N!)$ Backtracking.</li>\n                <li><strong>N ≤ 100–500:</strong> Cubic $O(N^3)$ or Quadratic $O(N^2)$ 2D DP / Floyd-Warshall.</li>\n                <li><strong>N ≤ 10^5:</strong> Linearithmic $O(N \\log N)$ Sorting, Binary Search, Heaps, or $O(N)$ Hash Map / Two Pointers.</li>\n                <li><strong>N ≥ 10^9:</strong> Logarithmic $O(\\log N)$ Binary Search / Bitwise or $O(1)$ Math.</li>\n              </ul>\n            ",
    "exampleTitle": "Fast Big-O Assessment Helper & Binary Search on Answer Pattern",
    "explanation": "Illustrates binary search over monotonic answer spaces with O(log(Max-Min) * N) complexity.",
    "timeComplexity": "O(N log(Range))",
    "spaceComplexity": "O(1)",
    "code": {
      "cpp": "// C++ Binary Search on Answer Template (e.g. Koko Eating Bananas)\n#include <vector>\n#include <algorithm>\n#include <numeric>\n\nbool canFinish(const std::vector<int>& piles, int speed, int h) {\n    long long hours = 0;\n    for (int p : piles) {\n        hours += (p + speed - 1) / speed; // ceil division\n    }\n    return hours <= h;\n}\n\nint minEatingSpeed(std::vector<int>& piles, int h) {\n    int low = 1, high = *std::max_element(piles.begin(), piles.end());\n    int ans = high;\n    while (low <= high) {\n        int mid = low + (high - low) / 2;\n        if (canFinish(piles, mid, h)) {\n            ans = mid;\n            high = mid - 1;\n        } else {\n            low = mid + 1;\n        }\n    }\n    return ans;\n}",
      "java": "// Java Binary Search on Answer Template\nimport java.util.Arrays;\n\npublic class ComplexityExample {\n    public static int minEatingSpeed(int[] piles, int h) {\n        int low = 1, high = Arrays.stream(piles).max().getAsInt();\n        int ans = high;\n        while (low <= high) {\n            int mid = low + (high - low) / 2;\n            if (canFinish(piles, mid, h)) {\n                ans = mid;\n                high = mid - 1;\n            } else {\n                low = mid + 1;\n            }\n        }\n        return ans;\n    }\n    private static boolean canFinish(int[] piles, int speed, int h) {\n        long hours = 0;\n        for (int p : piles) hours += (p + speed - 1) / speed;\n        return hours <= h;\n    }\n}",
      "python": "# Python Binary Search on Answer Template\nimport math\n\ndef min_eating_speed(piles: list[int], h: int) -> int:\n    def can_finish(speed: int) -> bool:\n        return sum(math.ceil(p / speed) for p in piles) <= h\n    \n    low, high = 1, max(piles)\n    ans = high\n    while low <= high:\n        mid = (low + high) // 2\n        if can_finish(mid):\n            ans = mid\n            high = mid - 1\n        else:\n            low = mid + 1\n    return ans\n"
    }
  },
  {
    "id": "patterns-playbook",
    "title": "28. Top 15 Algorithmic Patterns Recognition Framework",
    "category": "Interview Playbook",
    "summary": "A systematic mental flowchart to identify the exact algorithmic pattern in any unseen coding interview problem within 60 seconds.",
    "theory": "\n              <h3>Pattern Recognition Mental Flowchart</h3>\n              <ul>\n                <li><strong>Continuous subarray / contiguous substring with min/max length:</strong> ➔ <code>Sliding Window</code> (Fixed or Dynamic)</li>\n                <li><strong>Sorted array + find pair/triplet or palindrome:</strong> ➔ <code>Two Pointers</code> (Converging from ends)</li>\n                <li><strong>Linked list cycle or finding middle element:</strong> ➔ <code>Fast & Slow Pointers (Floyd's Tortoise & Hare)</code></li>\n                <li><strong>Next greater element, histogram area, stock span:</strong> ➔ <code>Monotonic Stack / Queue</code></li>\n                <li><strong>Top K elements / running median / stream processing:</strong> ➔ <code>Min/Max Heap (Priority Queue)</code></li>\n                <li><strong>Overlapping time intervals, meeting rooms:</strong> ➔ <code>Interval Sorting by Start/End Time</code></li>\n                <li><strong>Shortest path in unweighted graph or level-by-level tree traversal:</strong> ➔ <code>Breadth First Search (BFS)</code></li>\n                <li><strong>Connected components, cycle in undirected graph, dynamic connectivity:</strong> ➔ <code>Disjoint Set Union (DSU)</code></li>\n                <li><strong>Permutations, combinations, all subsets, Sudoku:</strong> ➔ <code>Backtracking with State Restoration</code></li>\n                <li><strong>Overlapping subproblems + optimal substructure (Min/Max cost, number of ways):</strong> ➔ <code>Dynamic Programming (1D / 2D)</code></li>\n              </ul>\n            ",
    "exampleTitle": "Monotonic Stack Next Greater Element Pattern",
    "explanation": "Maintains a decreasing monotonic stack to find the next greater element for all array entries in O(N) total time.",
    "timeComplexity": "O(N)",
    "spaceComplexity": "O(N)",
    "code": {
      "cpp": "// C++ Monotonic Decreasing Stack for Next Greater Element\n#include <vector>\n#include <stack>\n\nstd::vector<int> nextGreaterElements(const std::vector<int>& nums) {\n    int n = nums.size();\n    std::vector<int> res(n, -1);\n    std::stack<int> st; // stores indices\n    \n    for (int i = 0; i < n; ++i) {\n        while (!st.empty() && nums[st.top()] < nums[i]) {\n            res[st.top()] = nums[i];\n            st.pop();\n        }\n        st.push(i);\n    }\n    return res;\n}",
      "java": "// Java Monotonic Stack for Next Greater Element\nimport java.util.*;\n\npublic class PatternExample {\n    public static int[] nextGreaterElements(int[] nums) {\n        int n = nums.length;\n        int[] res = new int[n];\n        Arrays.fill(res, -1);\n        Deque<Integer> stack = new ArrayDeque<>();\n        \n        for (int i = 0; i < n; i++) {\n            while (!stack.isEmpty() && nums[stack.peek()] < nums[i]) {\n                res[stack.pop()] = nums[i];\n            }\n            stack.push(i);\n        }\n        return res;\n    }\n}",
      "python": "# Python Monotonic Stack for Next Greater Element\n\ndef next_greater_elements(nums: list[int]) -> list[int]:\n    n = len(nums)\n    res = [-1] * n\n    stack = []  # stores indices\n    \n    for i in range(n):\n        while stack and nums[stack[-1]] < nums[i]:\n            prev_idx = stack.pop()\n            res[prev_idx] = nums[i]\n        stack.append(i)\n        \n    return res\n"
    }
  }
];
