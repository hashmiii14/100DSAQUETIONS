...
class DSAApp {
  ...
  copyContactEmail() {
    ...
    const msg = '📋 Email copied: mdhashmi955@gmail.com';
  ...
  async handleContactSubmit(e) {
    ...
    const subject = (subjectEl && subjectEl.value.trim()) ? subjectEl.value.trim() : 'DSA Problems Feedback / Inquiry';
    ...
    try {
      if (typeof fetch === 'function') {
        const response = await fetch('https://formsubmit.co/ajax/mdhashmi955@gmail.com', {
          ...
          body: JSON.stringify({
            name: name || 'DSA Problems Learner',
            email: email || 'visitor@dsaproblems.site',
            _subject: `[DSA Problems] ${subject}`,
            message: message
          })
        });
      ...
      throw new Error('Fallback to direct Gmail Web Compose');
    } catch (err) {
      if (statusAlert) {
        statusAlert.style.display = 'block';
        statusAlert.style.background = 'rgba(59, 130, 246, 0.12)';
        statusAlert.style.border = '1px solid rgba(59, 130, 246, 0.4)';
        statusAlert.style.color = 'var(--text-primary)';
        statusAlert.innerHTML = `<strong>Opening Gmail Web...</strong> <a href="https://mail.google.com/mail/?view=cm&fs=1&to=mdhashmi955@gmail.com&su=${encodeURIComponent(subject)}&body=${encodeURIComponent('From: ' + name + ' (' + email + ')\n\n' + message)}" target="_blank" rel="noopener noreferrer" style="color: var(--accent-primary); text-decoration: underline; font-weight: 700;">Click here if Gmail did not open automatically</a>.`;
      }
      this.openInGmail();
      this.showToast('Opening pre-filled email in Gmail...');
    } finally {
      if (submitBtn) {
  ...
  openInGmail() {
    ...
    const name = nameEl ? nameEl.value.trim() : '';
    const email = emailEl ? emailEl.value.trim() : '';
    const subject = (subjectEl && subjectEl.value.trim()) ? subjectEl.value.trim() : 'DSA Problems Support & Feedback';
    const message = msgEl ? msgEl.value.trim() : '';

    const bodyText = message ? `From: ${name} (${email})\n\n${message}` : '';
    const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=mdhashmi955@gmail.com&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(bodyText)}`;
  ...
  renderGuideSection() {
    ...
    contentArea.innerHTML = `
      <div class="guide-article">
        ...
        ${selected.code ? `
          <div style="margin-top: 24px; padding-top: 20px; border-top: 1px solid var(--border-color);">
            <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px; margin-bottom: 12px;">
              ...
              <div class="guide-lang-tabs" style="display: flex; align-items: center; gap: 6px;">
                ...
                <button class="btn-copy-code" id="btn-copy-guide-code" onclick="app.copyGuideCode()" style="margin-left: 4px; padding: 4px 10px; font-size: 11.5px; border-radius: var(--radius-sm); border: 1px solid var(--border-color); background: var(--bg-surface); color: var(--text-primary); cursor: pointer; display: inline-flex; align-items: center; gap: 4px;">📋 Copy</button>
              </div>
            </div>
        ...
        <div style="margin-top: 24px; padding: 14px 16px; background: var(--bg-subtle); border: 1px solid var(--border-color); border-radius: var(--radius-md); display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;">
          <div>
            <strong style="font-size: 13.5px; color: var(--text-primary); display: block;">🚀 Master ${this.escapeHtml(selected.title.split('. ')[1] || selected.title)} with 1000 Problems</strong>
            <span style="font-size: 12px; color: var(--text-secondary);">Practice verified LeetCode problems covering this data structure & pattern.</span>
          </div>
          <button class="btn-primary" style="font-size: 12px; padding: 6px 14px;" onclick="app.navigate('/problems'); app.handleSearchInput('${(selected.title.split('. ')[1] || '').split(' ')[0].toLowerCase()}');">Practice in Sheet →</button>
        </div>

        <div class="guide-nav-controls" style="margin-top: 24px;">
  ...
  copyGuideCode() {
    ...
    const current = topics.find(t => t.id === this.activeGuideTopic) || topics[0];
    if (!current || !current.code) return;
    let code = '';
    if (typeof current.code === 'object') {
      code = current.code[this.activeGuideLang] || current.code.cpp || '';
    } else {
      code = current.code;
    }
    const btn = document.getElementById('btn-copy-guide-code');
    const updateBtn = () => {
      if (btn) {
    ...
    if (typeof navigator !== 'undefined' && navigator.clipboard && window.isSecureContext) {
      navigator.clipboard.writeText(code).then(() => {
        this.showToast(`📋 ${this.activeGuideLang.toUpperCase()} Guide code copied to clipboard!`);
        updateBtn();
      }).catch(() => {
        this.fallbackCopyText(code, 'Code copied to clipboard');
        updateBtn();
      });
    } else {
      this.fallbackCopyText(code, 'Code copied to clipboard');
  ...
  /* ── Daily DSA Challenge & Habit Streak Engine ────────────────────────────── */
  getDailyProblem() {
    const all = this.getProblems();
    ...
    const problemIdx = (dayOfYear * 7 + 13) % all.length;
    return all[problemIdx] || all[0];
  }

  getDailyStreak() {
    if (typeof localStorage === 'undefined') return 1;
    try {
      const data = JSON.parse(localStorage.getItem('dsaproblems_streak_v1') || '{}');
      const todayStr = new Date().toISOString().slice(0, 10);
      if (data.lastDate === todayStr) return data.count || 1;
      const yesterday = new Date();
      yesterday.setDate(yesterday.getDate() - 1);
      const yesterdayStr = yesterday.toISOString().slice(0, 10);
      if (data.lastDate === yesterdayStr) return data.count || 1;
      return 1;
    } catch (_) {
      return 1;
    }
  ...
  updateDailyChallengeWidget() {
    const p = this.getDailyProblem();
    if (!p) return;
    const titleEl = document.getElementById('daily-problem-title');
    const diffEl = document.getElementById('daily-problem-diff');
    ...
    if (diffEl) {
      ...
      diffEl.className = `diff-badge ${this.getDiffClass(p.difficulty)}`;
    }
    if (streakEl) {
      streakEl.textContent = this.getDailyStreak();
    }
    const btn = document.getElementById('btn-daily-solve');
    if (btn) {
      btn.innerHTML = isSolved ? '✓ Solved (View Solution)' : '💡 Solve Today\'s Problem';
    }
  }

  openDailyProblemModal() {
    const p = this.getDailyProblem();
    if (!p) return;
    this.openProblemModal(p.id);
  }

  /* ── Export Progress to CSV & Markdown ────────────────────────────────────── */
  exportProgressCSV() {
    const all = this.getProblems();
    const doneSet = this.state ? this.state.done : new Set();
    const bmSet = this.state ? this.state.bookmarked : new Set();

    let csvContent = 'ID,Title,Difficulty,Topic,Pattern,Status,Bookmarked,LeetCode URL\n';
    all.forEach(p => {
      const isSolved = doneSet.has(p.id) ? 'Solved' : 'Unsolved';
      const isBm = bmSet.has(p.id) ? 'Yes' : 'No';
      const title = `"${(p.title || '').replace(/"/g, '""')}"`;
      const topic = `"${(p.topic || '').replace(/"/g, '""')}"`;
      const pattern = `"${(p.pattern || '').replace(/"/g, '""')}"`;
      const url = p.leetcodeUrl || `https://leetcode.com/problems/${(p.slug || p.title.toLowerCase().replace(/[^a-z0-9]+/g, '-'))}/`;
      csvContent += `${p.id},${title},${p.difficulty},${topic},${pattern},${isSolved},${isBm},${url}\n`;
    });
    ...
    link.setAttribute('download', `DSAProblems_Progress_${new Date().toISOString().slice(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    this.showToast('📥 Progress exported to CSV successfully!');
  }
  ...
  exportProgressMarkdown() {
    ...
    const pct = ((solvedCount / totalCount) * 100).toFixed(1);

    let md = `# DSA Problems Mastery Sheet Progress\n\n`;
    md += `**Date:** ${new Date().toLocaleDateString('en-US', { dateStyle: 'full' })}\n`;
    md += `**Total Solved:** ${solvedCount} / ${totalCount} (${pct}% Completed)\n`;
    md += `**Study Resource:** [DSAProblems.site](https://www.dsaproblems.site/)\n\n`;
    md += `| # | Problem | Difficulty | Topic | Pattern | Status |\n`;
    md += `|---|---------|------------|-------|---------|--------|\n`;

    all.forEach(p => {
      const isSolved = doneSet.has(p.id) ? '✅ Solved' : '⏳ Unsolved';
      const url = p.leetcodeUrl || `https://leetcode.com/problems/${(p.slug || p.title.toLowerCase().replace(/[^a-z0-9]+/g, '-'))}/`;
      md += `| ${p.id} | [${p.title}](${url}) | ${p.difficulty} | ${p.topic} | ${p.pattern} | ${isSolved} |\n`;
    });
    ...
    const link = document.createElement('a');
    link.setAttribute('href', url);
    link.setAttribute('download', `DSAProblems_Study_Notes_${new Date().toISOString().slice(0, 10)}.md`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    this.showToast('📝 Progress exported to Markdown notes!');
  }
