...
# Precise FAANG 1,000 DSA Problem Bank Rebalancer & Generator
...
def main():
    print("Rebuilding 1,000 FAANG dataset with exact target topic caps...")
    ...
    final_by_category = {cat: [] for cat in TARGET_DISTRIBUTION.keys()}
    
    # Fill each category up to its exact target limit
    for cat, target_cnt in TARGET_DISTRIBUTION.items():
        existing_items = buckets[cat]
        selected = existing_items[:target_cnt]
        final_by_category[cat].extend(selected)
        
        needed = target_cnt - len(selected)
        if needed > 0:
            ...
            for i in range(needed):
                ...
                final_by_category[cat].append(new_prob)
        # Ensure exact cap at target_cnt
        final_by_category[cat] = final_by_category[cat][:target_cnt]
                
    # Combine all categories into one dataset
    all_balanced_problems = []
    for cat, items in final_by_category.items():
        all_balanced_problems.extend(items)
        print(f"Category '{cat}': {len(items)} problems (target: {TARGET_DISTRIBUTION[cat]})")
        
    print(f"Total balanced problems created: {len(all_balanced_problems)}")
    
    # Sort for easy-start foundation & interleaved progression
    easy_problems = [q for q in all_balanced_problems if q['difficulty'] == 'Easy']
    med_hard_problems = [q for q in all_balanced_problems if q['difficulty'] != 'Easy']
    
    # First 35 Easy problems
    ...
    # Assign sequential ID #0001 to #1000
    for idx, q in enumerate(curriculum, start=1):
        q['id'] = f"#{idx:04d}"
        
    print(f"Final rebalanced curriculum generated with exactly {len(curriculum)} problems!")
    ...
    print("Successfully wrote rebalanced 1,000 dataset to data/questions.js!")
